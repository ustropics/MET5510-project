...
for n_mode = 1:12
    ...
    XV_save = eigVec2(:, n_mode);  % Save original eigenvector
    ...
    fprintf('Mode %d: growth_rate = %.6e, phase_speed = %.2f m/s, e-folding = %.2f days\n', ...
        n_mode, growth_rate, phase_speed, eFolding);
    fprintf('Saved to: %s (%.1f MB)\n', calcFile, dir(calcFile).bytes/1e6);
end

fprintf('\n========== All 12 modes processed ==========\n');
toc
