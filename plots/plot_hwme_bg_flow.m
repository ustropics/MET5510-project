function plot_hwme_bg_flow(yy, zz, Ubar, BPVy, model, m0)
% PLOT_HWME_BACKGROUND_FLOW Plots the Hoskins-West Modified Eady model's background flow:
...
subplot(1, 3, 2);
interior_field = zeros(size(Ubar));  % Interior PV gradient is zero for HWME with no interior variation
...
caxis([-0.1 0.1]);  % Small range around zero
